...
Public Class DATA_HUTANG
    ...
    Sub loaddata()
        ...
        Dim dA As New FbDataAdapter("SELECT z.* FROM ( " _
                                        ...
                                        + " FROM TB_BELI a " _
                                        + " INNER JOIN TB_PARTNER b ON b.ID = a.SUPLIER LEFT JOIN (SELECT x.NOFAK,sum(x.BAYAR)AS BAYAR FROM TB_KAS_DET x GROUP BY x.NOFAK) c ON c.NOFAK = a.NOFAK" _
                                        + " UNION " _
                                        + " SELECT a.ID, a.LUNAS,a.NOFAK, a.TGL, b.NAMA, a.JT, -a.GRANDTOTAL,  " _
                                        + " CASE WHEN c.BAYAR IS NULL THEN 0 ELSE (c.BAYAR) END AS BAYAR, " _
                                        + " -a.GRANDTOTAL+(CASE WHEN c.BAYAR IS NULL THEN 0 ELSE c.BAYAR END) as SISA , a.CREATE_USERID, a.STAMP " _
                                        + " FROM TB_REBELI a " _
                                        + " INNER JOIN TB_PARTNER b ON b.ID = a.SUPLIER LEFT JOIN (SELECT x.NOFAK,sum(x.BAYAR)AS BAYAR FROM TB_KAS_DET x GROUP BY x.NOFAK) c ON c.NOFAK = a.NOFAK)z " _
    ...
    Private Sub DATA_HUTANG_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        namabarang()
        loaddata()
        carilunas.Text = "ALL"
        tgl1.Focus()
